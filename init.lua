...
minetest.register_globalstep(function(dtime)
    for i = 1, #wisp.wisps do
        ...
        local pos = wisp.wisps[i].pos
        local color = wisp.wisps[i].color
        if not c_wisp.move_time then
            c_wisp.move_time = 0
        end
        c_wisp.move_time = c_wisp.move_time + dtime
        if c_wisp.move_time > 2 then
            if random(3) > 1 then
        ...
        for n = 1, #objects do
            ...
            and obj:is_player() then
                ...
                pos.y = pos.y + 1.4
                local dir = vector.direction(pos, c_wisp.pos)
                local flee_dir = {
                    x = dir.x,
                    y = random(-1, 1),
                    z = dir.z
                }
                c_wisp.new_pos = vector.add(c_wisp.pos, flee_dir)
                ...
                and obj:get_wielded_item():get_name() == "wisp:jar" then
                    ...
                    if math.abs(vector.length(vector.subtract(dir, look_dir))) < 0.1 then
                        obj:set_wielded_item("wisp:jar_" .. c_wisp.color)
    ...
    for i = 1, #wisp.jars do
        ...
        or minetest.get_node(pos).name:match("^wisp:jar_") then
            ...
            if color == "red"
            and random(64) < 2 then
                local nodes = minetest.find_nodes_in_area_under_air(vector.subtract(pos, 5), vector.add(pos, 5), "group:flammable")
                if #nodes > 0 then
                    for n = 1, #nodes do
                        minetest.remove_node(nodes[n])
                        wisped_effect(nodes[n], color)
...
and minetest.get_modpath("nc_tree") then
    nodecore.register_craft({
		label = "assemble jar",
		normal = {y = 1},
		indexkeys = {"nc_optics:glass", "nc_tree:stick"},
		nodes = {
			{match = "nc_tree:stick", replace = "air"},
			{y = -1, match = "nc_optics:glass", replace = "wisp:jar"}
		}
	})
else
